...
- 不要放在这里：当前时间、cwd、工具列表、MCP instructions、运行时 UI 状态、
...
class ConstantSystemPrompt:
    ...
    SYSTEM_RULES_SECTION: str = (
        ...
        "limit. After compaction you'll see a `context_compaction` user message containing "
        "a handoff summary; use it as continuity context rather than a new request.\n"
    ...
    LOCAL_AGENT_GUIDANCE_SECTION: str = (
        ...
        "- 可并行且边界清晰的独立工作，优先用 agent 启动后台子代理；同一轮可以启动多个。\n"
        "- 子代理运行期间继续处理不重叠工作。系统会在后续模型轮次自动注入进度和完成通知。\n"
        "- 需要主动查看时使用 agent_task inspect；只有下一关键步骤必须依赖结果时才使用 wait。\n"

...
| `/cache` | 查看今天总计及按模型拆分的 prompt cache 命中率 |
...
`/cache` 读取 `.cbagent/logs/metrics/token-usage-YYYY-MM-DD.jsonl`，按 token 加权计算命中率；provider 返回 `prompt_cache_hit_tokens/prompt_cache_miss_tokens` 时使用精确分母，只有 `cached_tokens` 时用 `cached_tokens / prompt_tokens` 近似。

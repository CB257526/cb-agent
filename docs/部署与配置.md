...
- Bun `>=1.3.14`
- 剪贴板图片粘贴：Windows 使用 PowerShell/.NET Clipboard；macOS 默认使用系统自带 `osascript`，可选安装 `pngpaste` 兜底；Linux 推荐 `wl-paste` 或 `xclip`
...
> OTUI 会优先查找 `../venv/python.exe`、`../venv/Scripts/python.exe` 或 `../venv/bin/python`，所以推荐把虚拟环境放在项目父目录的 `venv`。
...
- OTUI 后端 stderr：`.cbagent/logs/system/gateway-<timestamp>.log`
...
`ui-otui/` 是基于 OpenTUI + Solid.js、运行在 Bun 上的唯一终端交互界面。
...
Python 路径可通过 `CB_AGENT_PYTHON` 覆盖，`CBAGENT_DANGEROUSLY_SKIP_PERMISSIONS` 会透传给后端。界面采用三栏布局：左侧消息流和输入框，右侧 Sidebar（会话/模型/MCP），底部 Footer 状态栏。
...
`bin/cbagent` 会从安装目录启动 OTUI，但把当前终端目录作为 agent 工作目录：

- `pwd` / `Get-Location` 会成为 Bash、文件读写、搜索工具的默认目录。
- 会话、日志、权限、短期记忆等项目状态写入当前目录的 `.cbagent/`。
- 模型配置默认仍读取安装目录的 `models.json`，不会读取当前目录的模型密钥文件。
- Python 环境仍按现有规则解析：`CB_AGENT_PYTHON` 优先，其次安装目录相邻的 `../venv`。
- 启动器会自动把 `../venv/bin`（Windows 为 `../venv/Scripts`）加入 `PATH`，所以 MCP stdio 命令、`python -m ...` 和 venv 内 console scripts 不需要手动激活 venv。

macOS / Linux：

```bash
chmod +x /path/to/cb-agent/bin/cbagent
ln -sfn /path/to/cb-agent/bin/cbagent ~/.bun/bin/cbagent
```
...
```powershell
# 假设项目位于 C:\Users\you\Desktop\cb-agent
$bin = "C:\Users\you\Desktop\cb-agent\bin"
...
如果 PowerShell 执行策略拦截 `.ps1`，可以直接运行 `cbagent.cmd`；脚本内部已使用 `-ExecutionPolicy Bypass` 启动配套 PowerShell 入口。
...
cd ui-otui && bun install && bun start
...
| 环境 | 依赖 | 说明 |
|---|---|---|
| macOS | 系统自带 `osascript`；可选 `pngpaste` | 默认无需额外安装，`pngpaste` 仅作为兜底 |

...
> **注意**：`LLM_MODEL_ID` 必须在 [constant/llm/constant_llm.py](../constant/llm/constant_llm.py) 的 `ConstantLLM.llm_dict` 里登记。新增模型时需要补全 `is_tool`、`is_reasoning`、`max_tokens`、`image_ability` 四个字段。

...
长期上下文遵守 canonical history 规则：模型实际看见的附件表示必须原样保存，不能在下一轮改写。通讯层日志和 UI 可以只保存脱敏摘要；原生视觉 data URI 的长期去重与外置存储留给 `ImageRef/MediaBlobStore` 独立重构。
